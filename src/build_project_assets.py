...
import numpy as np
import random, pickle, json  # noqa: E401
...
profiles = {
    'good': {
        'positive':
        lambda: norm.rvs(loc=norm.rvs(4), scale=1).astype(int),
        'negative':
        lambda: expon.rvs(loc=0, scale=np.random.choice([.5, 1])).astype(int),
        'chance':
        .5
    },
    'normal': {
        'positive':
        lambda: norm.rvs(loc=norm.rvs(3), scale=1).astype(int),
        'negative':
        lambda: norm.rvs(loc=2, scale=np.random.choice([.5, 1, 2, 3])).astype(
            int),
        'chance':
        .15
    },
    ...
    'chaotic_good': {
        'positive':
        lambda: left_skew(-1000, 5).astype(int),
        'negative':
        lambda: np.random.choice([0, np.random.choice([50, 200])],
                                 p=[.98, .02]),
        'chance':
        .2
    },
...
for employee_id in range(1, 26):
    ...
    employees[employee_id] = dict(employee_type=employee_type,
...
notes = pd.DataFrame(
    _, columns=['employee_id', 'employee_name', 'note']).assign(
...
notes = df.dropna()[['employee_id', 'team_id', 'note',
                     'event_date']].rename(columns={'event_date': 'note_date'})
